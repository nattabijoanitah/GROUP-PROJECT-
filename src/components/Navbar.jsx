...
export default function Navbar() {
  ...
  const [hoveredDropdownImg, setHoveredDropdownImg] = useState(null);
                    ...
                    >
                      {link.dropdown.map((sub, sIdx) => (
                        <Link
                          ...
                          onMouseEnter={sub.img ? () => setHoveredDropdownImg(sIdx) : undefined}
                          onMouseLeave={sub.img ? () => setHoveredDropdownImg(null) : undefined}
                          onTouchStart={sub.img ? () => setHoveredDropdownImg(sIdx) : undefined}
                          onFocus={sub.img ? () => setHoveredDropdownImg(sIdx) : undefined}
                          onBlur={sub.img ? () => setHoveredDropdownImg(null) : undefined}
                        >
                          {sub.name}
                        </Link>
                      ))}
                    ...
                    <div style={{ paddingLeft: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', borderLeft: '1px solid var(--border-color)', margin: '0.25rem 0 0.5rem 0' }}>
                      {link.dropdown.map((sub, sIdx) => (
                        <Link
                          ...
                          onTouchStart={sub.img ? () => setHoveredDropdownImg(`m${sIdx}`) : undefined}
                          onMouseEnter={sub.img ? () => setHoveredDropdownImg(`m${sIdx}`) : undefined}
                          onMouseLeave={sub.img ? () => setHoveredDropdownImg(null) : undefined}
                        >
                          {sub.name}
                        </Link>
